...
void st_wake_up()
{
  ...
  if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) { HAL_GPIO_WritePin(STEPPERS_DISABLE_GPIO_Port, STEPPERS_DISABLE_Pin, 0); }
...
void st_go_idle()
{
  ...
  if (pin_state) { HAL_GPIO_WritePin(STEPPERS_DISABLE_GPIO_Port, STEPPERS_DISABLE_Pin, 0); }
  else { HAL_GPIO_WritePin(STEPPERS_DISABLE_GPIO_Port, STEPPERS_DISABLE_Pin, 1); }
}
...
ISR(TIMER1_COMPA_vect)
{
  ...
  if (st.counter_y > st.exec_block->step_event_count) {
    st.step_outbits |= Y_STEP_Pin;
    #if defined(ENABLE_DUAL_AXIS) && (DUAL_AXIS_SELECT == Y_AXIS)
      st.step_outbits_dual = (1<<DUAL_STEP_BIT);
    #endif
    st.counter_y -= st.exec_block->step_event_count;
    if (st.exec_block->direction_bits & Y_DIRECTION_Pin) { sys_position[Y_AXIS]--; }
  ...
  if (st.counter_z > st.exec_block->step_event_count) {
    st.step_outbits |= Z_STEP_Pin;
    st.counter_z -= st.exec_block->step_event_count;
    if (st.exec_block->direction_bits & Z_DIRECTION_Pin) { sys_position[Z_AXIS]--; }
